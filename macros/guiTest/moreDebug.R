...
#### Function here to turn lats + cardinalities into 
    ...
    BMgradient <- aggregate(BMgradient, fact=20, fun=max)
    ...
    worldBM <- bedMap +
        geom_point(data = finalFrame, aes(x = easting, y = northing), size=3, color="red")
...
#### Hover
    ...
    output$hover_info <- renderUI({        
        ...
        point <- nearPoints(finalFrame, threshold=10000, maxpoints=1, hover, addDist = TRUE)
